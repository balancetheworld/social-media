...
export function ProfileHeader({ user }: { user: User }) {
  ...
  const joinMonth = joinDate.toLocaleDateString("zh-CN", { year: "numeric", month: "long" })
  ...
  return (
    <div>
          ...
          <div className="mb-2 flex items-center gap-2">
            {!isOwnProfile && isMutual && (
              <span className="badge-mutual">
                <Users className="h-2.5 w-2.5" /> 互关
              </span>
            )}
            {isOwnProfile ? (
        ...
        <div className="mt-2.5 flex flex-col gap-1.5">
          <div>
            <div className="flex items-center gap-1.5">
              ...
              {user.verified && (
                <svg className="h-[18px] w-[18px] text-primary" viewBox="0 0 22 22" fill="currentColor" aria-label="已认证">
          ...
          <div className="flex items-center gap-4 text-sm pb-3">
            <button onClick={() => setListType("following")} className="hover:underline transition-colors">
              ...
              <span className="text-muted-foreground ml-1 text-xs">正在关注</span>
            </button>
            <button onClick={() => setListType("followers")} className="hover:underline transition-colors">
              ...
              <span className="text-muted-foreground ml-1 text-xs">关注者</span>
            </button>
          </div>
        </div>
      </div>
      ...
      <FollowListDialog
        ...
        title={listType === "following" ? "正在关注" : "关注者"}
